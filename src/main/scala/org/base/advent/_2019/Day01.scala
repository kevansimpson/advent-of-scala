...
/**
  ...
  *
  * <b>What is the sum of the fuel requirements</b> for all of the modules on your spacecraft?
  *
  * <b>Part 2</b>
  ...
  * For example:
  * <ul>
  *   <li>A module of mass 14 requires 2 fuel. This fuel requires no further fuel (2 divided by 3 and rounded down is 0,
  *       which would call for a negative fuel), so the total fuel required is still just 2.</li>
  ...
  *       So, the total fuel required for a module of mass 1969 is 654 + 216 + 70 + 21 + 5 = 966.</li>
  *   <li>The fuel required by a module of mass 100756 and its fuel is:
  *       33583 + 11192 + 3728 + 1240 + 411 + 135 + 43 + 12 + 2 = 50346.</li>
  * </ul>
  *
  * <b>What is the sum of the fuel requirements</b> for all of the modules on your spacecraft when also taking into
  * account the mass of the added fuel? (Calculate the fuel requirements for each module separately, then add them all
  * up at the end.)
  */
class Day01 {
  private lazy val input = readLongs("/2019/input01.txt")

  def requiredFuel(mass: Long): Long = mass / 3L - 2L

  def fuelFuel(mass: Long): Long = {
    val fuel = requiredFuel(mass)
    if (fuel >= 0) fuelFuel(fuel) + fuel else 0
  }

  def solvePart1: Long = input.map(requiredFuel).sum

  def solvePart2: Long = input.map(fuelFuel).sum
}
