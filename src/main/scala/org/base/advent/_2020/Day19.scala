...
/**
  ...
  * rule 0; there can't be extra unmatched characters in the message. (For example, aaaabbb might appear to match
  * rule 0 above, but it has an extra unmatched b on the end.)
  *
  * <b>How many messages completely match rule 0?</b>
  *
  * <b>Part 2</b>
  ...
  * <ul>
  *     <li>bbabbbbaabaabba</li>
  *     <li>babbbbaabbbbbabbbbbbaabaaabaaa</li>
  *     <li>aaabbbbbbaaaabaababaabababbabaaabbababababaaa</li>
  *     <li>bbbbbbbaaaabbbbaaabbabaaa</li>
  *     <li>bbbababbbbaaaaaaaabbababaaababaabab</li>
  *     <li>ababaaaaaabaaab</li>
  *     <li>ababaaaaabbbaba</li>
  *     <li>baabbaaaabbaaaababbaababb</li>
  *     <li>abbbbabbbbaaaababbbbbbaaaababb</li>
  *     <li>aaaaabbaabaaaaababaa</li>
  *     <li>aaaabbaabbaaaaaaabbbabbbaaabbaabaaa</li>
  *     <li>aabbbbbaabbbaaaaaabbbbbababaaaaabbaaabba</li>
  * </ul>
  *
  * <b>After updating rules 8 and 11, how many messages completely match rule 0?</b>
  */
class Day19 {
  private lazy val input = readLines("/2020/input19.txt")

  type Rules = Map[String, String]

  def validMessages(input: Seq[String]): Long = valid(receive(input))

  def loopMessages(input: Seq[String]): Long = {
    val messages: Messages = receive(input)
    // h/t reddit/u/its_spelled_iain
    // https://www.reddit.com/r/adventofcode/comments/kg82yl/2020_day_19/ggdgsx7/?utm_source=reddit&utm_medium=web2x&context=3
    val updated: Rules = messages.rules +
  ...
  def toRegex(expr: String, rules: Rules): String = {
    if (expr.contains(" "))
      expr
        ...
        .map {
          case "|" => "|"
          case num => s"(${toRegex(rules.getOrElse(num, "x"), rules)})"
        }
        .mkString
  ...
  def receive(input: Seq[String]): Messages = {
    ...
    val map = span._1
      .map(rule => StringUtils.substringBefore(rule, ":") -> StringUtils.substringAfter(rule, ":").trim)
      .toMap
    Messages(map, span._2.tail)
  }

  def solvePart1: Long = validMessages(input)

  def solvePart2: Long = loopMessages(input)
}

case class Messages(rules: Map[String, String], received: Seq[String])
